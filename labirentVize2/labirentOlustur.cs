...
namespace labirentVize2
{
    public class Maze
    {
        ...
        public void Display(string a)
        {
            ...
            {
                var sbUst = new StringBuilder();
                var sbOrta = new StringBuilder();
                for (var x = 0; x < genislik; x++)
                {
                    if (x == 6)
                    {
                        sbUst.Append(this[x, y].HasFlag(HucreDurumu.Ust) ? "00" : "11");
                        sbOrta.Append(this[x, y].HasFlag(HucreDurumu.Sol) ? "01" : "11");
                    }

                    else
                    {
                        sbUst.Append(this[x, y].HasFlag(HucreDurumu.Ust) ? "010" : "011");

                        sbOrta.Append(this[x, y].HasFlag(HucreDurumu.Sol) ? "011" : "111");
                    }

                }
                if (firstLine == string.Empty)
                    firstLine = sbUst.ToString();
                // Console.WriteLine(sbUst + "0");
                // Console.WriteLine(sbOrta + "0");
                //Console.WriteLine(sbOrta + "0");

                a += (sbUst + "0" + sbOrta + "0" + sbOrta + "0").ToString();
